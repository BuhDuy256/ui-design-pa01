...
**Giác quan và hệ vận động**: Time/Distance/Speed đặt giữa màn hình dưới dạng số lớn, tương phản cao, phù hợp với vùng foveal vision — nơi mắt nhìn rõ chi tiết nhất — nên một lần liếc nhanh khi đang đi bộ là đủ để đọc. Ngược lại, xác nhận Pause/Resume/Finish chỉ dựa vào kênh thị giác (đổi màu/nhãn nút); một tín hiệu haptic (rung) sẽ phù hợp hơn ở đây vì mắt đang bận quan sát đường.

**Mô hình tư duy và ẩn dụ tương tác**: Start → Pause/Resume → Finish khớp với mô hình tư duy quen thuộc ("đi bộ rồi dừng"); biểu tượng ▶/❚❚ và cờ ca-rô là ẩn dụ tương tác mượn từ trình phát nhạc và đua xe, không cần học riêng. Tuy nhiên, kỳ vọng "Finish = xong" không được đáp ứng: bốn màn hình thêm (Save, "Nice work!", popup thành tích, Share) xuất hiện trước khi đạt trạng thái ổn định — cho thấy luồng thao tác chưa tuân theo Shneiderman Rule #4 (design dialogs to yield closure).

**Khả năng sử dụng**: Learnability cao trong lần trải nghiệm này — vòng lặp cốt lõi được thao tác đúng ngay từ đầu, không cần hướng dẫn. Efficiency tốt trong lúc ghi, nhưng giảm rõ ngay sau Finish do bốn màn hình phụ không được yêu cầu. Errors không phát sinh trong phần ghi hoạt động.
...
**Chunking**: dữ liệu hiển thị dạng khối số liệu lớn và biểu đồ thay vì bảng dày đặc, giúp giảm số lượng chunk cần quét cùng lúc. Tuy nhiên, một số chỉ số có tên gần giống nhau nằm sát cạnh nhau mà không có nhãn phân biệt.

**Recognition vs Recall**: Avg Pace, Avg Elapsed Pace, Moving Time, Elapsed Time hiển thị mà không có gợi ý trực quan giải thích điều gì phân biệt chúng, khiến việc hiểu ý nghĩa dựa vào nhớ lại (recall) thay vì nhận diện (recognition) — trong khi nguyên tắc là ưu tiên nhận diện. Cách đặt tên gần giống nhau ("Elapsed Pace" cạnh "Pace", "Elapsed Time" cạnh "Moving Time") càng làm tăng gánh nặng ghi nhớ này.

**Khả năng sử dụng**: Learnability thấp cho riêng màn hình này — các nhãn khó phân biệt nếu không có trợ giúp bên ngoài. Errors không phát sinh trực tiếp (màn hình chỉ đọc), nhưng có nguy cơ nhầm Moving Time với Elapsed Time khi so sánh các buổi tập.
...
**Thử thách và thanh tiến độ**: thanh tiến độ ở tab Active cho các thử thách đã tham gia là một dạng feedback trực tiếp, hiển thị liên tục mức độ hoàn thành — cơ chế phù hợp để duy trì động lực tham gia. Thẻ thử thách ("July 5K Challenge") giới thiệu mục tiêu cụ thể, dễ nhận diện hơn là phải tự đặt mục tiêu. Tuy nhiên, sau khi Join, không có đường dẫn nào để xem lại chi tiết hay tiến độ của thử thách vừa tham gia — phản hồi duy nhất là nút đổi nhãn, cho thấy một khoảng trống về feedback: đã tham gia nhưng không rõ theo dõi ở đâu. Bên cạnh đó, công cụ tự tạo thử thách ("Design Your Own Challenge") — cơ chế chủ động nhất để tự đặt mục tiêu cạnh tranh — chỉ khả dụng với gói trả phí, nên phần lớn trải nghiệm động lực trong tab Active đến từ thử thách có sẵn do Strava đưa ra.

**Thành tích và xã hội**: ngoài các thành phần xuất hiện trực tiếp trong tab Groups, Strava còn dùng popup thành tích (quan sát ở Tình huống sử dụng 1) và banner Kudos (Tình huống sử dụng 2) như những cơ chế bổ sung nhằm duy trì động lực sau mỗi hoạt động. Nhưng trong phiên trải nghiệm này, các cơ chế đó chưa thấy kết nối rõ với các thử thách hay câu lạc bộ ở tab Groups — động lực cá nhân và động lực cộng đồng có vẻ vận hành như hai luồng tách rời.

**Kết nối cộng đồng**: mục tiêu cụ thể "tìm câu lạc bộ gần khu vực" không có điểm truy cập nào trên tab Clubs; chỉ có nội dung quảng bá (tạo câu lạc bộ riêng, "The Strava Club" chính thức). Không có tính năng khám phá theo vị trí hay hoạt động nào hiển thị, làm giảm khả năng khám phá cộng đồng ngay từ màn hình đầu tiên (Visibility).

**Join và trang chi tiết**: sự chú ý tập trung vào nút Join trước khi phần còn lại của thẻ được đọc; sau khi chạm, chỉ nhãn nút đổi, không mở được chi tiết. Đây là biểu hiện cụ thể, ở quy mô một tương tác đơn lẻ, của cùng khoảng trống feedback nêu trên — chứ không phải vấn đề trung tâm của tab Groups.

**Cảm nhận chung**: trong phạm vi phiên trải nghiệm này, hai mục tiêu chính — theo dõi thử thách đã tham gia, và tìm câu lạc bộ — đều chưa đạt được, làm giảm cảm nhận về giá trị cộng đồng và động lực mà tab Groups hướng tới, dù các cơ chế khuyến khích riêng lẻ (thanh tiến độ, thành tích) cho thấy Strava có đầu tư vào việc này.
