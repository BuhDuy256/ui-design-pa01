...
- **Người mới bắt đầu**: phù hợp với trải nghiệm này — sự nhầm lẫn xảy ra ngay trong lần đầu tiếp cận màn hình, đặc biệt với nhóm nhãn gần giống nhau (Avg Pace / Avg Elapsed Pace).
...
- **Người lớn tuổi**: màn hình chấp nhận được về mặt thao tác — chỉ cần cuộn và chạm, không có nhiệm vụ nhập liệu nào. Tuy nhiên, các chỉ số trong biểu đồ tốc độ (đặc biệt cụm Avg Pace / Avg Elapsed Pace quan sát từ IMG_0527) được hiển thị với cỡ chữ nhỏ hơn so với các chỉ số chính — có thể gây khó đọc hơn với người có thị lực kém.
- **Người khuyết tật**: màn hình chỉ đọc và bố cục một cột, không có cơ chế nhập liệu phức tạp — trình đọc màn hình có thể hoạt động tốt nếu các nhãn được gán đúng. Nhưng nếu nhãn không phân biệt được ngay cả khi nhìn, người dùng trình đọc màn hình cũng sẽ gặp đúng vấn đề tương tự như người dùng thông thường: nhãn nghe giống nhau, không rõ ý nghĩa khác nhau.
- **Bối cảnh sử dụng**: màn hình này được xem sau khi hoạt động kết thúc, trên cùng chiếc iPhone, trong cùng phiên — người dùng đang ngồi yên, chú ý không bị chia. Đây là điều kiện thuận lợi hơn nhiều so với UC1 (ngoài trời, đang di chuyển). Vấn đề về nhãn không phải do môi trường mà do thiết kế: cụm nhãn gây nhầm lẫn dù người dùng có toàn bộ thời gian để đọc.
...
- **Người mới bắt đầu**: phù hợp với phiên này — không tìm được đường vào chi tiết thử thách sau khi Join, và không tìm được câu lạc bộ gần khu vực trên tab Clubs — cả hai mục tiêu chính đều không đạt được.
...
- **Người lớn tuổi**: nội dung tab Groups chủ yếu là thẻ và thanh tiến độ — không có thao tác nhập liệu hay cử chỉ phức tạp. Về mặt vận động, tab này không đặt ra rào cản đặc biệt. Vấn đề khó tìm câu lạc bộ và thiếu phản hồi sau Join có thể ảnh hưởng mọi nhóm tuổi như nhau.
- **Người khuyết tật**: tab Groups có cấu trúc dạng danh sách (thẻ, thanh tiến độ) phù hợp với trình đọc màn hình nếu nội dung được đánh nhãn đúng. Tuy nhiên, việc nút Join đổi nhãn mà không có thông báo bổ sung có nghĩa là người dùng trình đọc màn hình cũng sẽ không nhận được xác nhận rõ ràng sau khi tham gia thử thách — cùng khoảng trống feedback đã nêu.
- **Bối cảnh sử dụng**: phiên khám phá tab Groups diễn ra riêng biệt với buổi đi bộ — người dùng đang ngồi yên, không xử lý nhiệm vụ gấp. Điều này có nghĩa là những khó khăn quan sát được (không tìm được câu lạc bộ, không rõ theo dõi thử thách sau Join) xuất hiện ngay cả trong điều kiện thư giãn, không có áp lực thời gian — khả năng cao là chúng sẽ rõ hơn trong tình huống có nhu cầu thực (ví dụ: tìm câu lạc bộ trước khi chạy).
